...
from utils.tools import random_bbox, mask_image, get_config, mask_image2
...
for model in models:
    ...
    for j in range(img_idx):
        ...
        x, mask, orig = mask_image2(np.array([field]), bboxes, config, bnd=config['boundary'])
        # print(x.shape)
        # Plot box made
        ...
        netG = Generator(config['netG'], True, config['gpu_ids'])
